...
引用の先頭行を`コラム`で始めると、通常の引用とは異なる専用デザイン（見出しバー付きのボックス）で表示されます。**マーカー行と本文の間には必ず空の`>`行を入れてください**（入れないと1つの段落として扱われ、コラム判定に失敗します）。
...
- `コラム`の後の`:`または`：`以降がサブタイトルとして見出しバーに表示されます（省略可）
- 本文は複数段落でもOK（引き続き空行区切りの`>`段落として書く）
